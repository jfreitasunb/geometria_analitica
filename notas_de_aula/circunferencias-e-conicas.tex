...
Como no caso da elipse, utilizando a simetria e as assíntodas da hipérbole, podemos mostrar a partir da equação
\[
...
Agora, efetuando-se uma rotação no sentido antihor\'ario nos eixos $x$ e $y$ de um ângulo $\theta$, obtemos novos eixos coordenados $x_1$ e $y_1$. Tome vetores unit\'atios $\vec{u_1}$ e $\vec{u_1}$ nos eixos $x_1$ e $y_1$. Em relação aos eixos originais podemos escrever
\begin{figure}[!h]
  \centering
...
Queremos remover o termo $xy$ da equação \eqref{equacao_geral_conica} para então usar uma translação de eixos, se necess\'ario, e com isso identicar a c\^onica definida por tal equação. Para isso vamos tentar encontrar um ângulo $\theta$ tal que a equação \eqref{equacao_geral_conica} possa ser transformada numa equação da forma
