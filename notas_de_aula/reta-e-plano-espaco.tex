...
\begin{equation}
    \vec{AP} = t\vec{v}
\end{equation}
onte $t \in \real$ e $P$ é um ponto qualquer de $r$. Sendo $A$ um ponto em $\real^3$ e $\vec{v}$ um vetor de $\real^3$ temos
\begin{align*}
    A = (x_0, y_0, z_0)\\
    \vec{v} = (a, b, c).
\end{align*}
...
\begin{enumerate}
\item Se $\vec{u}\varparallel\vec{v}$, então $r$ e $s$ são paralelas ou coincidentes. Elas serão coincidentes se, e somente se, possuirem algum ponto em comum.\index{Retas!Paralelas}
\begin{figure}[h]
    \centering
...
\begin{exemplos}
    ...
    \begin{enumerate}
        ...
        \begin{solucao}
            A interseção será dada pela solução do sitema
            \[
                \begin{cases}
                    x - 2y + 3z = 2\\
                    3x + y - 2z = 1\\
