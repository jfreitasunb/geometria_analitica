...
Como os ponto $P_0$, $P_1$ e $P_2$ n\~ao s\~ao colineares, este sistema sempre possui solu\c{c}\~ao \'unica.
...
E como a elipse \'e sim\'etrica, sua forma ser\'a:
\begin{figure}
  \centering
...
\begin{figure}[!h]
  ...
  \end{pspicture*}
\end{figure}
Esse ret\^angulo \'e chamado de \textbf{ret\^angulo fundamental} da elipse $E$.\index{Elipse!Ret\^angulo Fundamental}
...
Agora, dado um ponto $A(\alpha,\beta)$ que satisfaz a equa\c{c}\~ao \eqref{equacaohiperbole}, queremos mostrar que $A$ pertence \`a hip\'erbole $H$. Temos
\begin{align}
  \dfrac{\alpha^2}{a^2} - \dfrac{\beta^2}{b^2} = 1\nonumber\\
...
\begin{observacao}
  ...
  \begin{enumerate}
    ...
    \item O ponto $(x,0)$ pertence \`a hip\'erbole $H$ se, e s\'o se, $x = \pm a$. Por isso a interse\c{c}\~ao de $H$ com a reta focal s\~ao os pontos $A_1(-a,0)$ e $A_2(a,0)$, chamados de \textbf{v\'ertices} de $H$.\index{Hip\'erbole!V\'ertices}
  \end{enumerate}
\end{observacao}
...
Seja $P(x_0,y_0)$ um ponto de interse\c{c}\~ao entre $r$ e $H$. Ent\~ao temos
\begin{align*}
...
\subsection{Par\'abolas} % (fold)
\label{sub:parabolas}

\begin{definicao}
  Seja $r$ uma reta e $F$ um ponto que n\~ao pertence a $r$. O lugar geom\'etrico $P$ dos pontos equidistantes de $F$ e $r$ chama-se \textbf{par\'abola}. O ponto $F$ \'e chamado de \textbf{foco da par\'abola} e $r$ de \textbf{reta diretriz}.\index{Par\'abola} \index{Par\'abola!Focos} \index{Par\'abola!Reta diretriz}\todo{Definir reta focal.}
\end{definicao}
...
A equa\c{c}\~ao \eqref{equacaoparabola} \'e chamada de \textbf{equa\c{c}\~ao reduzida} da par\'abola $P$. Indica-se\index{Par\'abola!Equa\c{c}\~ao reduzida}
\[
  P:\ y^2 = 4px.
\]
...
\begin{observacao}
  \begin{enumerate}
    ...
    \item O \'unico ponto de interse\c{c}\~ao de $P$ com os eixos coordenados \'e o centro. O ponto $(0,0)$ \'e chamado de \textbf{v\'ertice} da par\'abola.\index{Par\'abola!V\'ertice}
  \end{enumerate}
\end{observacao}
...
Na equa\c{c}\~ao \eqref{equacaoconica} pelo menos um dos n\'umeros $a$, $b$ $c$ \'e diferente de zero. Os termos $ax^2$ e $by^2$ s\~ao os \textbf{termos quadr\'aticos}, o termo $cxy$ \'e o \textbf{termo quadr\'atico misto}, $dx$ e $ey$ s\~ao os \textbf{termos lineares} e $f$ \'e o \textbf{termo independente}. \index{C\^onica!Termos Quadr\'aticos} \index{C\^onica!Termo Quadr\'atico Misto} \index{C\^onica!Termos Lineares} \index{C\^onica!Termos Independete}
...
Agora, efetuando-se uma rota\c{c}\~ao no sentido antihor\'ario nos eixos $x$ e $y$ de um \^angulo $\theta$, onde $0 \le \theta \le \pi$,\todo{conferir restrição no ângulo} obtemos novos eixos coordenados $x_1$ e $y_1$. Tome vetores unit\'atios $\vec{u_1}$ e $\vec{u_1}$ nos eixos $x_1$ e $y_1$. Em rela\c{c}\~ao aos eixos originais podemos escrever
\begin{figure}[!h]
  \centering
